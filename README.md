...
- **2026-05-30** [#28](https://github.com/tomollow/lbmcode/pull/28): sec3 の [`src/sec3/lbmtherm.c`](src/sec3/lbmtherm.c) に解析模式図 [`docs/assets/sec3/lbmtherm_schematic.png`](docs/assets/sec3/lbmtherm_schematic.png) と格子細分化収束研究 [`docs/assets/sec3/lbmtherm_convergence.png`](docs/assets/sec3/lbmtherm_convergence.png) を追補。$n_x = n_y \in \{32, 48, 64, 80, 96\}$ を残差 norm $\le 10^{-9}$ まで走らせ、Bouzidi–Firdaouss–Lallemand (Phys Fluids 13, 3452, 2001) の理論 2 次精度に対し実測収束次数 $\approx 1$（$L_2$ 傾き $-1.04$, $L_\infty$ 傾き $-0.95$）であることを正直に報告（線形化 D2Q5 平衡分布 / 熱境界向け公式 / 移流支配条件の 3 つを推察として記載）。[`scripts/plot_lbmtherm_schematic.py`](scripts/plot_lbmtherm_schematic.py), [`scripts/run_lbmtherm_convergence.py`](scripts/run_lbmtherm_convergence.py) と CSV [`docs/sec3/generated/lbmtherm_convergence.csv`](docs/sec3/generated/lbmtherm_convergence.csv) を同梱。C ソースは無変更
- **2026-05-30** [#27](https://github.com/tomollow/lbmcode/pull/27): sec1/sec2/sec4/sec5/sec6 にセクション概要 README を追加（sec3 と同じ雛形: ファイル一覧表 + ビルド/実行 + スクリプト一覧 + 「このディレクトリで扱う物理」）。各ディレクトリ ([`src/sec1/README.md`](src/sec1/README.md), [`src/sec2/README.md`](src/sec2/README.md), [`src/sec4/README.md`](src/sec4/README.md), [`src/sec5/README.md`](src/sec5/README.md), [`src/sec6/README.md`](src/sec6/README.md)) に直接降りても章のスコープが把握できる構成
- **2026-05-30** [#26](https://github.com/tomollow/lbmcode/pull/26): `secN/*.c` を解説・実行・ベンチマーク・PR まで一気通貫で処理する Claude Code skill [`.claude/skills/lbm-analyze/SKILL.md`](.claude/skills/lbm-analyze/SKILL.md) を追加。docs/scripts/assets の出力先、図番号規約、`subprocess.run` の `timeout`/`shell=False` 必須化、`(?m)^[ \t]*` 行頭アンカ、`try/finally` 一時ファイル削除などのガードを skill 規約として固定
- **2026-05-30** [#25](https://github.com/tomollow/lbmcode/pull/25): sec3 の 2 サンプルを文書化。double-population thermal LBM による側面加熱正方キャビティ自然対流 [`src/sec3/lbmnc.c`](src/sec3/lbmnc.c) を新規追加し、$Ra = 10^3, 10^4, 10^5, 10^6$ を de Vahl Davis (1983) ベンチマークと比較（[`docs/sec3/lbmnc.md`](docs/sec3/lbmnc.md), [`scripts/run_lbmnc_ra_sweep.py`](scripts/run_lbmnc_ra_sweep.py), CSV 3 種）。[`src/sec3/lbmtherm.c`](src/sec3/lbmtherm.c) 側は閉形式の advection-diffusion 解析解と直接比較する [`docs/sec3/lbmtherm.md`](docs/sec3/lbmtherm.md) を新設。sec3 ディレクトリ概要 README ([`src/sec3/README.md`](src/sec3/README.md)) も同梱
- **2026-05-27** [#23](https://github.com/tomollow/lbmcode/pull/23): §4.6 図 4.5（Geier 2006 カスケード LBM）の **定性再現** として sec4 に [`src/sec4/lbmcm_karman.c`](src/sec4/lbmcm_karman.c) を追加。[`lbmcm.c`](src/sec4/lbmcm.c) の中心モーメント衝突と [`karman.c`](src/sec4/karman.c) の体積力 + x 周期駆動を組み合わせ、長方形障害物 4×16 lu のウェイクを 256×80 格子で $\tau=0.503$（SRT/MRT 発散域）で安定に走らせて $\mathrm{Re}_D \approx 1094$ を達成。プロット [`scripts/plot_lbmcm_karman_spectrum.py`](scripts/plot_lbmcm_karman_spectrum.py) で速度場 $|\mathbf{U}|$ とウェイク窓 26 フレームの radial 平均 $E(k)$ を Kolmogorov $-5/3$ 参照線と並べ、説明ドキュメント [`docs/sec4/lbmcm_karman.md`](docs/sec4/lbmcm_karman.md) と一発実行スクリプト [`scripts/run_lbmcm_karman.ps1`](scripts/run_lbmcm_karman.ps1) も同梱。あわせて [`scripts/build_one.cmd`](scripts/build_one.cmd) のデフォルトに `/O2` を追加して全 LBM ビルドの実行速度を改善
