...
Karman は同じ $\nu_t/\nu_0$ レベルでも極端に大きな抑制を見せる — これは渦放出が**振動現象**で、平均流ではなく振動振幅を減衰させる k-ε の特性が劇的に現れるため。kelbm の中位（1.6）は「壁関数で $k$ を強制注入する低 Re 平均流」というポジションを反映。

## 設計上の共通パターン

全 6 ケースで再利用された実装パターン：
...
- **対流項の離散化はケース依存**: 平均流れ（kelbm, cavity, step, karman）は 1 次風上で安定性優先、滑らかな周期場（TG, KH）は中心差分で精度優先。拡散項はすべて 2 次中心

### Runner / Plot
- **`scripts/run_<name>.ps1`**: build → run pure / k-eps → plot 生成、`-PureOnly`/`-KepsOnly`/`-SkipPlot` 対応。両者同時指定の guard は KH 以降の runner（kelvin_helmholtz, cavity, backward_step, karman）で導入（TG runner は guard なし — 時系列の都合）
