...
- 数値解と解析解の比較結果、格子細分化による収束次数

## 解析モデル

![lbmtherm schematic](../assets/sec3/lbmtherm_schematic.png)

図 3.3　lbmtherm.c の解析モデル。(a) 周期チャネル: 上下壁の温度は $T_{\mathrm{wall}}(x) = \cos(k x)$ で空間的に変調され、内部では一様水平速度 $\mathbf{u} = (u_0, 0)$ が与えられる（速度場は LBM で解かず外部から与える）。$x$ 方向は周期境界。(b) サブグリッド壁オフセット $q$: 壁面は最隣接 ghost 行から距離 $q$（lattice 単位）だけ離れた位置にあり、$q = 0.5$ のとき通常の半過程 bounce-back に一致する。$q \neq 0.5$ では 3 セル分の補間 bounce-back（[lbmtherm.c:239-247](../../src/sec3/lbmtherm.c#L239-L247) など）で壁面値を補間する（公式自体は 2 次精度を意図して構築されているが、本コードでの実測収束次数は次節を参照）。(c) $Pe = 20$ における解析解の温度モード $T_a(x, y)$。複素波数 $\beta$ の符号により、波形が下流（$x$ 方向）に進むほど位相がずれていく様子が見える。
...
図 3.4　lbmtherm.c による advection-diffusion 検証結果。(a) D2Q5 LBM の数値解 $T$、(b) 解析解 $T_a$、(c) 差分 $T - T_a$（相対 $L_2$ 誤差は 5.88 %）。下段 (d) $x = 0.016\,L,\ 0.266\,L,\ 0.516\,L$ の鉛直断面プロファイル（実線が数値解、破線が解析解）、(e) 内部点での誤差分布のヒストグラム。誤差は壁近傍に集中しており、これは $q = 0.7$ のサブグリッド壁オフセットを補間 bounce-back で処理する際の打ち切り誤差が支配的であることを示している（収束次数の実測は次節を参照）。
...
補間 bounce-back の理論的な空間収束次数は、Bouzidi–Firdaouss–Lallemand (Phys. Fluids 13, 3452, 2001) が直線壁・球状壁などに対して 2 次オーダーであることを示しています。この理論値を本コードの設定 ($Pe = 20$, $q = 0.7$, $\tau_g = 0.56$, Dirichlet 余弦温度境界) で実測するため、$n_x = n_y \in \{32, 48, 64, 80, 96\}$ で格子細分化スイープを行いました。

各実行で時間残差を十分に下げる（最終 norm が $10^{-9}$ 以下）よう、外側ループ回数を $\max(200,\ 200 \cdot (n_x/64)^2)$ にスケールしています。詳細は [scripts/run_lbmtherm_convergence.py](../../scripts/run_lbmtherm_convergence.py)、生データは [docs/sec3/generated/lbmtherm_convergence.csv](generated/lbmtherm_convergence.csv) を参照。

> **物理問題のわずかな揺らぎ**: コード中の $h = n_y - 2 + 2q$（$q = 0.7$）は $n_y$ にほぼ比例するが厳密には一致しないため、波数あたりの Peclet 数 $Pe_{\mathrm{wave}} = u_0 / (\chi k) = Pe \cdot n_x / (2\pi h)$ がスイープ間で 1 % 程度ドリフトする（$n_x = 32$ で 3.244、$n_x = 96$ で 3.203）。観測される収束次数 $\approx 1$ への寄与は無視できる量。

![lbmtherm convergence](../assets/sec3/lbmtherm_convergence.png)

図 3.5　補間 bounce-back の空間収束。(a) $L_2$ 相対誤差・$L_\infty$ 絶対誤差を $1/n_x$ に対して両対数で示し、最小二乗フィットの傾きを凡例に表示。点線が 1 次精度、一点鎖線が 2 次精度の参考線（$n_x = 64$ を通るように規格化）。(b) 隣接する 2 解像度から計算した局所収束次数。$L_2$ で約 $1.04$、$L_\infty$ で約 $0.95$ と、いずれも理論の 2 次精度ではなく 1 次精度に近い挙動を示す。

### 収束次数表

| $n_x$ | $h$ (lattice) | total steps | 相対 $L_2$ | $L_\infty$ | 観測次数 $p_{L_2}$ |
| ---: | ---: | ---: | ---: | ---: | ---: |
| 32 | 31.4 | 100000 | $1.308\times 10^{-1}$ | $5.92\times 10^{-2}$ | — |
| 48 | 47.4 | 100000 | $8.55\times 10^{-2}$ | $4.04\times 10^{-2}$ | 1.05 |
| 64 | 63.4 | 100000 | $6.32\times 10^{-2}$ | $3.07\times 10^{-2}$ | 1.04 |
| 80 | 79.4 | 156000 | $5.04\times 10^{-2}$ | $2.48\times 10^{-2}$ | 1.02 |
| 96 | 95.4 | 225000 | $4.19\times 10^{-2}$ | $2.07\times 10^{-2}$ | 1.02 |

> **時間収束との関係**: 既定設定（$n_x = 64$, 50000 ステップ）の $L_2 = 5.88\\%$ は時間定常に到達する手前の値である。同じ格子で 100000 ステップまで延長すると残差 norm は $\sim 10^{-7}$ → $\sim 10^{-9}$ まで下がり、$L_2$ は $6.32\\%$ に収束する（上表）。図 3.4 の 5.88 % と本節の 6.32 % のずれはこの時間残差の差に対応する。

### 1 次精度に留まる理由（推察）

理論 2 次精度を実現できていない原因として、以下が考えられます。

- [lbmtherm.c:188](../../src/sec3/lbmtherm.c#L188) の平衡分布が $O(u)$ のみを保持する線形化形 $g_k^{\mathrm{eq}} = w_k T (1 + 3 \mathbf{c}_k\cdot\mathbf{u})$ で書かれており、$O(u^2)$ 補正項を含まない。等温拡散（$u_0 = 0$）なら 2 次精度を回復する可能性があり、追加検証の余地がある。
- 補間 bounce-back の係数（[lbmtherm.c:239-247](../../src/sec3/lbmtherm.c#L239-L247)）は Bouzidi らの流体境界向けではなく、Seta 系の熱境界向け版に準じている。熱用と流体用で公式が異なるため、文献の 2 次精度結果がそのまま適用できるとは限らない。
- $Pe = 20$ の比較的高い移流支配条件で、壁近傍の境界層が薄い。$L_2$ ノルムの分母には全体平均が入るため、薄い境界層の誤差が大域評価上に強く反映され、収束次数が見かけ上落ちている可能性もある。

実装上の課題というよりは、教材コードの「補間 bounce-back の効果」と「公式選択の影響」を体感する素材として扱うのが妥当です。
...
- サブグリッド壁を扱う interpolated bounce-back の係数 $(2q-1)/(2q+1)$ などが、コード上に直接書かれているため、係数の意味と導出に立ち入った勉強がしやすい。公式自体は 2 次精度になるよう構築されているが、本コードでは線形化平衡分布や移流支配の影響で観測次数が約 1 に留まる（格子細分化結果は前節）。
