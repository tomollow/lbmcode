...
// epsp: parameter for collision force between partcle0 and particle1
// epsw: parameter for collision force between partcles and walls
...
{
  FILE   *fp;
  int    nx = 100, ny = 800, time = 0, loop1, loop2;
//  int    nx = 50, ny = 400, time = 0, loop1, loop2;
//  int    nx = 50, ny = 500, time = 0, loop1, loop2;
  ...
  double epsp = 10.0, epsw = 10.0, zeta = 1.0;
...
// particle
  ...
  xp[0] = 0.999/2.0*(double)nx;
  yp[0] = 7.200/2.0*(double)nx;
  xp[1] = 1.000/2.0*(double)nx;
  yp[1] = 6.800/2.0*(double)nx;
...
// particle-particle collision
    for(n = 0; n < np; n++){ for(k = n+1; k < np; k++){
      tmp1 = sqrt((xp[n] - xp[k])*(xp[n] - xp[k])
                 +(yp[n] - yp[k])*(yp[n] - yp[k]));
      if(tmp1 < 2.0f*rp + zeta){
        fxc[n] += (xp[n] - xp[k])*(2.0f*rp - tmp1 + zeta)*(2.0f*rp - tmp1 + zeta)/epsp;
        fyc[n] += (yp[n] - yp[k])*(2.0f*rp - tmp1 + zeta)*(2.0f*rp - tmp1 + zeta)/epsp;
...
// right wall 
      tmp1 = fabs(xp [n]- (nx - 1 + rp));
      if(tmp1 < 2.0f*rp + zeta){
        fxc[n] += (xp[n] - (nx - 1 + rp))*(2.0f*rp - tmp1 + zeta)*(2.0f*rp - tmp1 + zeta)/epsw;
      }
// top wall 
//      tmp1 = fabs(yp[n] - (ny - 1 + rp)); 
//      if(tmp1 < 2.0f*rp + zeta){
//        fyc[n] += (yp[n] - (ny - 1 + rp))*(2.0f*rp - tmp1 + zeta)*(2.0f*rp - tmp1 + zeta)/epsw;
//      }
    } 
...
// particle motion
  ...
  for(j = ny - 1; j >= 1; j = j - 6){ for(i = 1; i <= nx - 1; i = i + 6){
    printf("%c",a[i][j]);
    }
    printf("\n");
  }
  ...
  printf("time : %f\n",time*dt);
  printf("y0:%f, y0:%f, u0:%f, v0:%f, theta0:%f\n",xp[0]*2.0/(double)nx,yp[0]*2.0/(double)nx,up[0],up[0],theta[0]);
  printf("x1:%f, x1:%f, u1:%f, v1:%f, theta1:%f\n",xp[1]*2.0/(double)nx,yp[1]*2.0/(double)nx,vp[1],vp[1],theta[1]);
