...
// overcome destruction — the model decays toward chi << c_v1 and nu_t goes
...
void init_wall_distance() {
    for (int y = 0; y < NY; ++y) {
        for (int x = 0; x < NX; ++x) {
            ...
            // Required floor: the staircase boundary doesn't exactly coincide
            // with the analytical radius R_CYL, so corner fluid cells can have
            // sqrt(dx^2+dy^2) - R_CYL slightly less than 0.5 (or even negative).
            // Without this floor d_tilde -> 0 and the SA destruction term
            // C_W1*fw*(nt/d_tilde)^2 blows up.
...
int output_snapshot(int step) {
    char fname[64];
    snprintf(fname, sizeof(fname), "karman_des_snapshot_%05d.csv", step);
...
int main() {
    ...
    FILE* hist = fopen("karman_des_probe.csv", "w");
    if (!hist) {
        fprintf(stderr, "main: cannot open karman_des_probe.csv\n");
        return 1;
    }
    ...
    printf("Done. Snapshots: karman_des_snapshot_*.csv, probe: karman_des_probe.csv\n");
