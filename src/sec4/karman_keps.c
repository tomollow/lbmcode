// karman_keps.c
// 2D Karman vortex street + standard k-epsilon overlay (D2Q9, BGK)
//
// Same geometry, body force, and asymmetric initial v perturbation as karman.c.
// k-eps transport drives nu_t = Cmu*k^2/eps; the local tau_eff = 0.5 + 3(nu0+nu_t)
// in BGK damps the shedding compared to pure DNS — by how much depends on how
// strongly the wake activates Pk.
//
// Wall functions (Dirichlet) on top wall, bottom wall, and on every fluid cell
// adjacent to the cylinder. The cylinder boundary is staircase, so the
// "tangential speed" is taken as the local |u| (treating the wall as having
// zero velocity in any orientation).
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
#define WALL_DY 0.5
#define KEPS_DT 0.05
#define U_TYPICAL 0.07               // representative speed for k, eps initial seed
...
void update_kepsilon() {
    ...
    // Wall functions: top, bottom, and cylinder surface (any fluid cell with a
    // solid orthogonal neighbor). apply_wall_function takes max so corner-like
    // cells touched by both top/bottom wall and cylinder don't get clobbered.
    for (int x = 0; x < NX; ++x) {
    ...
    for (int y = 0; y < NY; ++y) {
        for (int x = 0; x < NX; ++x) {
            int i = IDX(x, y);
            if (solid[i]) continue;
            int adjacent_to_solid = 0;
            int ixp = (x+1)%NX, ixm = (x-1+NX)%NX;
            if (solid[IDX(ixp, y)]) adjacent_to_solid = 1;
            if (solid[IDX(ixm, y)]) adjacent_to_solid = 1;
            if (y+1 < NY && solid[IDX(x, y+1)]) adjacent_to_solid = 1;
            if (y-1 >= 0 && solid[IDX(x, y-1)]) adjacent_to_solid = 1;
            if (adjacent_to_solid) {
                // Total speed as proxy for tangent on staircase boundary
                double speed = sqrt(u[i]*u[i] + v[i]*v[i]);
                apply_wall_function(i, speed);
            }
        }
    }
...
int output_snapshot(int step) {
    char fname[64];
    snprintf(fname, sizeof(fname), "karman_keps_snapshot_%05d.csv", step);
    ...
    compute_vorticity();
    fprintf(fp, "x,y,u,v,vorticity,solid,k,eps,nut\n");
    for (int y = 0; y < NY; ++y) {
        for (int x = 0; x < NX; ++x) {
            int i = IDX(x, y);
            fprintf(fp, "%d,%d,%.9g,%.9g,%.9g,%d,%.9g,%.9g,%.9g\n",
...
int main() {
    ...
    FILE* hist = fopen("karman_keps_probe.csv", "w");
    if (!hist) {
        fprintf(stderr, "main: cannot open karman_keps_probe.csv\n");
        return 1;
    }
    fprintf(hist, "step,u_max,u_probe,v_probe,k_mean,eps_mean,nut_mean\n");
    ...
    for (int t = 0; t < NSTEPS; ++t) {
        ...
        if (t % HISTORY_INTERVAL == 0) {
            ...
            fprintf(hist, "%d,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g\n",
                    t, umax, u[p], v[p], k_sum*inv_n, eps_sum*inv_n, nut_sum*inv_n);
        }
        update_kepsilon();
    ...
    printf("Done. Snapshots: karman_keps_snapshot_*.csv, probe: karman_keps_probe.csv\n");
