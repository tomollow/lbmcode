// cavity_les.c
// 2D lid-driven cavity + standard Smagorinsky LES (D2Q9, BGK)
//
// Same geometry and boundary conditions as cavity.c (4-wall halfway BB with
// the top wall moving at (U_LID, 0)). Smagorinsky SGS: nu_t = (Cs*Delta)^2 * |S|,
// Cs = 0.16, Delta = 1 (grid spacing in LU). nu_t couples back to BGK via
// tau_eff = 0.5 + 3*(nu_0 + nu_t).
//
// No wall damping. Standard Smagorinsky over-predicts nu_t near solid
// boundaries because |S| stays finite at the first off-wall cell — for a
// recirculating laminar cavity this is acceptable: nu_t is uniformly tiny.
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
void update_les() {
    ...
    for (int y = 0; y < NY; ++y) {
        for (int x = 0; x < NX; ++x) {
            int i = IDX(x, y);
            // One-sided differences at walls
...
int output_snapshot(int step) {
    char fname[64];
    snprintf(fname, sizeof(fname), "cavity_les_snapshot_%05d.csv", step);
    ...
    fprintf(fp, "x,y,u,v,vorticity,psi,nut\n");
    for (int y = 0; y < NY; ++y) {
        for (int x = 0; x < NX; ++x) {
            int i = IDX(x, y);
            fprintf(fp, "%d,%d,%.9g,%.9g,%.9g,%.9g,%.9g\n",
                    x, y, u[i], v[i], vort[i], psi[i], nut_field[i]);
        }
    }
    fclose(fp);
    return 0;
...
int main() {
    ...
    FILE* hist = fopen("cavity_les_history.csv", "w");
    if (!hist) {
        fprintf(stderr, "main: cannot open cavity_les_history.csv\n");
        return 1;
    }
    fprintf(hist, "step,u_max,v_max,psi_min,nut_mean\n");
    ...
    for (int t = 0; t < NSTEPS; ++t) {
        ...
        if (t % HISTORY_INTERVAL == 0) {
            double umax = 0, vmax = 0, nut_sum = 0;
            ...
            fprintf(hist, "%d,%.9g,%.9g,%.9g,%.9g\n",
                    t, umax, vmax, psi_min, nut_sum*inv_n);
        }
        update_les();
    ...
    printf("Done. Snapshots: cavity_les_snapshot_*.csv, history: cavity_les_history.csv\n");
    printf("Parameters: NX=%d NY=%d NSTEPS=%d TAU=%.3f U_LID=%.3f Cs=%.3f nu0=%.5f Re=%.0f\n",
           NX, NY, NSTEPS, TAU, U_LID, CS_SMAG, nu0, Re);
    return 0;
}
