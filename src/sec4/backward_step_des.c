// backward_step_des.c
// 2D backward-facing step + Spalart-Allmaras DES97 (D2Q9, BGK)
//
// Same channel + step geometry, body force, and periodic-x boundary as
// backward_step.c / backward_step_les.c. SA-DES hybrid: 1-equation transport
// for nu_tilde with length-scale switch
//
//     d_tilde = min(d_wall, C_DES * Delta),   C_DES = 0.65, Delta = 1 LU
//
// Wall distance combines all four boundary segments of the L-shaped fluid
// domain (channel ceiling, channel floor downstream of step, step top face,
// step right face). The step convex corner is handled with a Euclidean
// distance to (STEP_LENGTH-0.5, STEP_HEIGHT-0.5).
//
// At Re_H ~ 56 the recirculation is laminar and SA production cannot
// overcome destruction. nu_t collapses to ~ 10^{-11} (f_v1 cutoff) and the
// reattachment length matches pure LBM exactly — same "DES sleeps in
// laminar regime" story as cavity_des / karman_des.
...
void init_wall_distance() {
    for (int y = 0; y < NY; ++y) {
        for (int x = 0; x < NX; ++x) {
            ...
            double d_top = NY - y - 0.5;
            double d_bot = y + 0.5;
            double d_step;
            if (x < STEP_LENGTH) {
                // Above the step: distance to the step top face
                d_step = y - STEP_HEIGHT + 0.5;
            } else if (y < STEP_HEIGHT) {
                // Right of the step (recirculation region): distance to step right face
                d_step = x - STEP_LENGTH + 0.5;
            } else {
                // Past the convex corner: Euclidean distance to corner point
                double dxc = x - (STEP_LENGTH - 0.5);
                double dyc = y - (STEP_HEIGHT - 0.5);
                d_step = sqrt(dxc*dxc + dyc*dyc);
            }
            double d = d_top < d_bot ? d_top : d_bot;
            if (d_step < d) d = d_step;
            d_wall[i] = d;
...
int output_snapshot(int step) {
    char fname[64];
    snprintf(fname, sizeof(fname), "step_des_snapshot_%05d.csv", step);
    ...
    fprintf(fp, "x,y,u,v,vorticity,psi,solid,nut,nu_tilde,d_wall,d_tilde\n");
    for (int y = 0; y < NY; ++y) {
        for (int x = 0; x < NX; ++x) {
            int i = IDX(x, y);
            fprintf(fp, "%d,%d,%.9g,%.9g,%.9g,%.9g,%d,%.9g,%.9g,%.9g,%.9g\n",
...
int main() {
    ...
    FILE* hist = fopen("step_des_history.csv", "w");
    if (!hist) {
        fprintf(stderr, "main: cannot open step_des_history.csv\n");
        return 1;
    }
    fprintf(hist, "step,u_max,reattach_x,nut_mean,nu_tilde_mean,les_frac\n");
    ...
    for (int t = 0; t < NSTEPS; ++t) {
        ...
        if (t % HISTORY_INTERVAL == 0) {
            ...
            fprintf(hist, "%d,%.9g,%d,%.9g,%.9g,%.9g\n",
                    t, umax, reattach, nut_sum*inv_n, nt_sum*inv_n, les_frac);
        }
        update_sa_des();
    ...
    printf("Done. Snapshots: step_des_snapshot_*.csv, history: step_des_history.csv\n");
    printf("Parameters: NX=%d NY=%d STEP=%dx%d NSTEPS=%d TAU=%.3f F=%g C_DES=%.3f nu0=%.5f u_max=%.4f Re_H=%.0f\n",
           NX, NY, STEP_LENGTH, STEP_HEIGHT, NSTEPS, TAU, FORCE_X, C_DES, nu0, umax_actual, Re_H);
    printf("DES branch: LES fraction = %.4f over fluid cells (RANS layer = 1 cell at all walls)\n", les_frac);
    return 0;
}
