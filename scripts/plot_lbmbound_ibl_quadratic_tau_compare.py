...
def run_case(executable: Path, tau: float) -> Path:
    output_dir = OUTPUT_DIR / f"lbmbound_ibl_quadratic_tau_{str(tau).replace('.', '_')}"
...
def main() -> None:
    ...
    y_over_h = (np.arange(1, NY, dtype=float) - 1.0 + Q) / H
    ...
    profile_axis.set_title("Quadratic 補間 bounce back 境界での定常速度分布比較")
    ...
    output_path = OUTPUT_DIR / "lbmbound_ibl_quadratic_tau_compare.png"
    published_path = PUBLISHED_ASSET_DIR / "lbmbound_ibl_quadratic_tau_compare.png"
