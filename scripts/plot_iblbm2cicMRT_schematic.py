"""Analytical-model schematic for src/sec6/iblbm2cicMRT.c.
...
MRT collision operator and the Guo (2002) forcing term.
...
marker points on each ring (sparser than the SRT sibling: ne factor 0.2),
the annular gap, the rotation arrows and the analytical tangential-velocity
profile.

Outputs:
    outputs/sec6/iblbm2cicMRT/iblbm2cicMRT_schematic.png
    docs/assets/sec6/iblbm2cicMRT_schematic.png
"""
...
def main() -> None:
    ...
    draw_schematic(axis)
    axis.set_title(
        r"図 6.2 円筒 Couette 流の解析モデル（陰的速度補正 IB-LBM, MRT）",
        fontsize=12,
    )

    PUBLISHED_ASSET_DIR.mkdir(parents=True, exist_ok=True)
    RUN_DIR.mkdir(parents=True, exist_ok=True)
    for path in (RUN_DIR / "iblbm2cicMRT_schematic.png",
                 PUBLISHED_ASSET_DIR / "iblbm2cicMRT_schematic.png"):
