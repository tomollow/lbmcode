"""Result figure for src/sec6/iblbm2cdfSRT.c (cylindrical Couette, IB-LBM).

Reads the converged interior velocity fields produced by

    cmd /c scripts\\run_one.cmd src\\sec6\\iblbm2cdfSRT.c

from outputs/sec6/iblbm2cdfSRT/ and builds a 4-panel figure:
...
Also writes the radial benchmark table to
    docs/sec6/generated/iblbm2cdfSRT_couette_profile.csv
...
CSV_PATH = ROOT_DIR / "docs" / "sec6" / "generated" / "iblbm2cdfSRT_couette_profile.csv"
...
def main() -> None:
    ...
    fig.suptitle(
        f"図 6.1 円筒 Couette 流の IB-LBM 解 (相対 L2 誤差 err = {rel_l2:.4f})",
        fontsize=13)

    PUBLISHED_ASSET_DIR.mkdir(parents=True, exist_ok=True)
    for path in (RUN_DIR / "iblbm2cdfSRT_results.png",
                 PUBLISHED_ASSET_DIR / "iblbm2cdfSRT_results.png"):
