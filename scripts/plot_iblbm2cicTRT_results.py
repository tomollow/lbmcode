"""Result figure for src/sec6/iblbm2cicTRT.c (cylindrical Couette, IB-LBM).

Reads the converged interior velocity fields produced by

    cmd /c scripts\\run_one.cmd src\\sec6\\iblbm2cicTRT.c

from outputs/sec6/iblbm2cicTRT/ and builds a 4-panel figure:
...
Also writes the radial benchmark table to
    docs/sec6/generated/iblbm2cicTRT_couette_profile.csv
and prints the relative L2 error that the C code reports as `err`.

This is the TRT + implicit-velocity-correction sibling of
plot_iblbm2cicMRT_results.py; the field layout and analytic profile are
...
CSV_PATH = ROOT_DIR / "docs" / "sec6" / "generated" / "iblbm2cicTRT_couette_profile.csv"
...
def main() -> None:
    ...
    fig.suptitle(
        f"図 6.5 円筒 Couette 流の IB-LBM 解 (陰的補正・TRT, 相対 L2 誤差 err = {rel_l2:.4f})",
        fontsize=13)

    PUBLISHED_ASSET_DIR.mkdir(parents=True, exist_ok=True)
    for path in (RUN_DIR / "iblbm2cicTRT_results.png",
                 PUBLISHED_ASSET_DIR / "iblbm2cicTRT_results.png"):
