"""Analytical-model schematic for src/sec6/iblbm2cdfSRT.c.

Cylindrical (circular) Couette flow solved with a direct-forcing
immersed-boundary lattice Boltzmann method (IB-LBM, SRT collision).
...
marker points on each ring, the annular gap, the rotation arrows and the
analytical tangential-velocity profile.

Outputs:
    outputs/sec6/iblbm2cdfSRT/iblbm2cdfSRT_schematic.png
    docs/assets/sec6/iblbm2cdfSRT_schematic.png
"""
...
RUN_DIR = ROOT_DIR / "outputs" / "sec6" / "iblbm2cdfSRT"
PUBLISHED_ASSET_DIR = ROOT_DIR / "docs" / "assets" / "sec6"

# Source parameters (iblbm2cdfSRT.c).
NX = NY = 50
...
def main() -> None:
    ...
    draw_schematic(axis)
    axis.set_title(
        r"図 6.0 円筒 Couette 流の解析モデル（直接強制型 IB-LBM, SRT）",
        fontsize=12,
    )

    PUBLISHED_ASSET_DIR.mkdir(parents=True, exist_ok=True)
    RUN_DIR.mkdir(parents=True, exist_ok=True)
    for path in (RUN_DIR / "iblbm2cdfSRT_schematic.png",
                 PUBLISHED_ASSET_DIR / "iblbm2cdfSRT_schematic.png"):
