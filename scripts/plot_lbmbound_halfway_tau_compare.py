...
def run_case(executable: Path, tau: float) -> Path:
    output_dir = OUTPUT_DIR / f"lbmbound_halfway_tau_{str(tau).replace('.', '_')}"
...
def main() -> None:
    ...
    y_over_h = (np.arange(1, H + 1, dtype=float) - 0.5) / H
    ...
    profile_axis.set_title("Half-way bounce back 境界での定常速度分布比較")
    ...
    output_path = OUTPUT_DIR / "lbmbound_halfway_tau_compare.png"
    published_path = PUBLISHED_ASSET_DIR / "lbmbound_halfway_tau_compare.png"
